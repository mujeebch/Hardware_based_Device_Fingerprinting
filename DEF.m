...
function [filling_lit_model,filling_lit_measured]=DEF(lit,count,col,run) % data_extract_filling
...
while(i<=length(lit)) % loop to read whole string of data for given sensor
    ...
    if (i>201) % just bc in if we are checking if its filling or emptying, to set logic for that
        if (lit(i)>=930) % earlier was 920 due to exchange data moved to this value, this condition to set flag for lower value to be used in following loop
            a=1;
            while (a==1) && (lit(i)<971) && (lit(i) - lit(i-100)>0) % this loop will run for desired data points
                ...
                filling_lit_model(run,col) = lit_ini + (0.45500326-0.42157445); % getting a particular row and all col elements for a vector
